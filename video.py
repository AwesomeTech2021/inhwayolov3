...
def write(x, results):
    c1 = tuple(x[1:3].int())	#x의 1~2
    c2 = tuple(x[3:5].int())    #x의 3~4
    img = results		# 사진원본, 추후 박스가 이 함수를 지날때마다 생김
    cls = int(x[-1])		# 클래스의 약자, 어떤 객체인지에 대한 정보가 담김
    color = colors[cls%100]	# 같은 객체 같은 색, 을 위한 코
    
    center_object_x, center_object_y = [(c1[0]+c2[0]) / 2, (c1[1]+c2[1]) / 2]
    
    global location
    if center_object_x < center_x and center_object_y > center_y: 
       location = "left down"
    elif center_object_x > center_x and center_object_y > center_y:
       location = "right down"
    elif center_object_x < center_x and center_object_y < center_y:
       location = "left up"
    elif center_object_x > center_x and center_object_y < center_y:
    ...
    label = "{0}: {1}, {2}".format(classes[cls],str(obj_counter[cls]), location)
...
while True:
    ...
    if ret:   
        ...
        # 새로운 배열을 만들고 write 할떄마다 이 배열에 모든 객체의 위치좌표를 등록
